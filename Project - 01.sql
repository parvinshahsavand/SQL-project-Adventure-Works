...
-- 3
-- میانگین فاصله زمانی بین تمام سفارش های
-- هر مشتری که حداقل درسال 2015 تعداد 10 سفارش ثبت کرده باشد را به روز حساب کنید
...
IF @Country IS NULL AND @EmpName IS NULL 
BEGIN
	SELECT 
	...
	WHERE O.orderdate >= @DateFrom 
		...
		--AND E.firstname + ' ' + E.lastname = @EmpName
		AND DATEDIFF (DAY, O.requireddate , O.orderdate) < @Days
END
ELSE IF @Country IS NULL AND @EmpName IS NOT NULL 
BEGIN
	SELECT 
	O.orderdate --Sales.Orders: orderdate
	, O.requireddate -- Sales.Orders: requireddate
	...
	WHERE O.orderdate >= @DateFrom 
		...
		AND DATEDIFF (DAY, O.requireddate , O.orderdate) < @Days
END
ELSE IF @Country IS NOT NULL AND @EmpName IS NULL 
BEGIN
	SELECT 
	O.orderdate --Sales.Orders: orderdate
	, O.requireddate -- Sales.Orders: requireddate
